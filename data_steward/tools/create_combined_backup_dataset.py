...
def assert_tables_in(client: BigQueryClient, dataset_id: str):
    ...
    LOGGER.info(f'Dataset {dataset_id} has tables: {tables}')
...
def create_cdm_tables(client: BigQueryClient, combined_dataset_id: str):
    ...
    for table in resources.CDM_TABLES:
        ...
        client.create_table(dest_table)  # Make an API request.
...
def query(client: BigQueryClient,
    ...
    query_job = client.query(q, job_config)
...
def ehr_consent(client: BigQueryClient, rdr_dataset_id: str,
    ...
    :return:
    """
    q = combine_consts.EHR_CONSENT_QUERY.format(
...
def mapping(client: BigQueryClient, domain_table: str, rdr_dataset: str,
    ...
    for key in combine_consts.FOREIGN_KEYS_FIELDS:
        if key in fields_to_join:
            if domain_table == combine_consts.PERSON_TABLE:
                ...
                table_alias = mapping_table_for('{x}'.format(x=key)[:-3])
                join_expression.append(
                    combine_consts.LEFT_JOIN.format(dataset_id=combined_dataset,
                                                    prefix=key[:3],
                                                    field=key,
                                                    table=table_alias))
...
def load_fact_relationship(client: BigQueryClient, rdr_dataset: str,
    ...
    :return: None
    """
    q = combine_consts.FACT_RELATIONSHIP_QUERY.format(
        rdr_dataset_id=rdr_dataset,
        combined_dataset_id=combined_dataset,
...
def main(raw_args=None):
    ...
    client.copy_dataset(f'{args.vocab_dataset}', f'{combined_dataset}')
    LOGGER.info(
    ...
    LOGGER.info(f'Translating {combine_consts.PERSON_TABLE} table from EHR...')
    ...
    for domain_table in combine_consts.DOMAIN_TABLES:
        ...
        mapping(client, domain_table, args.rdr_dataset,
                args.unioned_ehr_dataset, combined_dataset)
    for domain_table in combine_consts.DOMAIN_TABLES:
